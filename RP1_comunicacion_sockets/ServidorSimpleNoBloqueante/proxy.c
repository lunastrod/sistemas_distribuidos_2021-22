...
int accept_new_client(int sockfd) {
  int connfd =
      accept(sockfd, (struct sockaddr *)NULL, NULL);  // Acepta un nuevo cliente
  if (connfd < 0) {
...
void send_recv(int sockfd) {
  ...
  printf("> ");
  if (select(sockfd + 1, &fds, NULL, NULL, &tv) <0) {  // sokfd+1 comprueba todos los valores menores que ese, comprueba
    // stdin tambien
    err(1, "select error");
  }
  printf("\n");
